...
Panel {
  ...
  // Hero popup icon, updated with each successful weather response.
  property string label: ""
  // Compact bar read-out: temperature always; UV in daylight; rain only
  // while it is falling or due within the next few hours.
  readonly property string barTempLabel: reportTempNum !== "" ? reportTempNum + "°" : ""
  ...
  readonly property var uvSeries:           Model.openMeteoTodayHourlyUv(dailyForecastReport, Qt.formatDate(now, "yyyy-MM-dd"))
  readonly property var hourlyPrecip:       Model.openMeteoHourlyPrecip(dailyForecastReport)
  readonly property real nowHour:           now.getHours() + now.getMinutes() / 60
  readonly property string reportUv: {
    var direct = current ? Model.formatUv(current.uvIndex) : ""
    if (direct !== "") return direct
    if (uvSeries.length > 0) return Model.formatUv(Model.uvAtHour(uvSeries, nowHour))
    return ""
  }
  readonly property var uvChartLayout:      Model.uvChartLayout(uvSeries, nowHour, uvChart.width, uvChart.height, 5)
      ...
      Row {
        ...
        spacing: 0
          ...
          Item {
            ...
            Rectangle {
              visible: root.uvChartLayout.hasNow
              width: Style.space(7)
              height: width
              radius: width / 2
              color: Style.selectedStateColor(root.bar.foreground, Color.accent)
              x: root.uvChartLayout.nowX - width / 2
